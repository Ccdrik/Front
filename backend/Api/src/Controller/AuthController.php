...
public function signup(Request $request, EntityManagerInterface $em, UserPasswordHasherInterface $hasher): JsonResponse
{
    ...
    $pseudo = $data['pseudo'] ?? null;  // <-- ajouté

    if (!$email || !$password || !$confirmation || !$nom || !$prenom || !$pseudo) { // <-- pseudo ajouté
    ...
    $user->setPseudo($pseudo);  // <-- ajouté
    $user->setRoles(['ROLE_PASSAGER']); // Ou ROLE_USER selon ta logique
